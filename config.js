...
module.exports = require('yargs')
    ...
    .check(argv => {
        if (argv.payload !== 'plain' && argv.payload !== 'json') {
            throw new Error('payload type must be "plain" or "json"');
        }
        if (typeof argv.w1Interval !== 'number' || argv.w1Interval < 1) {
            throw new Error('w1-interval has be a number greater than 0');
        }
        if (typeof argv.w1Wait !== 'number') {
            throw new TypeError('w1-wait has to be a number');
        }
        return true;
    })
